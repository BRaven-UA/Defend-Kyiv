...
void fragment(){
	...
	vec2 direction0 = normalize(crater_uv0 - half);
	vec2 direction1 = normalize(crater_uv1 - half);
	vec2 direction2 = normalize(crater_uv2 - half);
	vec2 direction3 = normalize(crater_uv3 - half);
	vec2 direction4 = normalize(crater_uv4 - half);
	vec2 direction5 = normalize(crater_uv5 - half);
	vec2 direction6 = normalize(crater_uv6 - half);
	vec2 direction7 = normalize(crater_uv7 - half);
	vec2 direction8 = normalize(crater_uv8 - half);
	vec2 direction9 = normalize(crater_uv9 - half);
	vec2 direction10 = normalize(crater_uv10 - half);
	vec2 direction11 = normalize(crater_uv11 - half);
	vec2 direction12 = normalize(crater_uv12 - half);
	vec2 direction13 = normalize(crater_uv13 - half);
	vec2 direction14 = normalize(crater_uv14 - half);
	vec2 direction15 = normalize(crater_uv15 - half);
	vec2 direction16 = normalize(crater_uv16 - half);
	vec2 direction17 = normalize(crater_uv17 - half);
	vec2 direction18 = normalize(crater_uv18 - half);
	vec2 direction19 = normalize(crater_uv19 - half);
	...
	// distortion distance
	vec2 dist0 = crater0.r * direction0;
	vec2 dist1 = crater1.r * direction1;
	vec2 dist2 = crater2.r * direction2;
	vec2 dist3 = crater3.r * direction3;
	vec2 dist4 = crater4.r * direction4;
	vec2 dist5 = crater5.r * direction5;
	vec2 dist6 = crater6.r * direction6;
	vec2 dist7 = crater7.r * direction7;
	vec2 dist8 = crater8.r * direction8;
	vec2 dist9 = crater9.r * direction9;
	vec2 dist10 = crater10.r * direction10;
	vec2 dist11 = crater11.r * direction11;
	vec2 dist12 = crater12.r * direction12;
	vec2 dist13 = crater13.r * direction13;
	vec2 dist14 = crater14.r * direction14;
	vec2 dist15 = crater15.r * direction15;
	vec2 dist16 = crater16.r * direction16;
	vec2 dist17 = crater17.r * direction17;
	vec2 dist18 = crater18.r * direction18;
	vec2 dist19 = crater19.r * direction19;
	...
	vec4 craters = clamp(crater0 + crater1 + crater2 + crater3 + crater4 + crater5 + crater6 + crater7 + crater8 + crater9 + crater10 + crater11 + crater12 + crater13 + crater14 + crater15 + crater16 + crater17 + crater18 + crater19, 0.0, 0.2);
	vec4 screen = texture(TEXTURE, UV - crater_dists * crater_strength + shockwaves, 0.0);
	COLOR = mix(screen, craters.ggga, craters.a); // combine distorted sceen with crater texture
}