...
void fragment()
{
	...
	float isWhite = smoothstep(0.35, 0.36, uv.y);
	float isBlue = smoothstep(0.62, 0.63, uv.y);
	vec3 col = mix(vec3(1.0), vec3(0.0, 0.0, 1.0), isWhite);
	col = mix(col, vec3(1.0, 0.0, 0.0), isBlue);
	
	vec4 tex = texture(decal, uv / vec2(0.25, 0.4) - vec2(1.55, 0.73));
